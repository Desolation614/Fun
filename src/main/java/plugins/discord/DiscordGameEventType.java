...
import net.runelite.client.plugins.discord.DiscordAreaType;
...
enum DiscordGameEventType {
    ...
    private DiscordGameEventType(Skill skill) {
        this(skill, 0);
    }

    private DiscordGameEventType(Skill skill, int priority) {
        this.details = DiscordGameEventType.training(skill);
        this.priority = priority;
        this.imageKey = DiscordGameEventType.imageKeyOf(skill);
        this.shouldTimeout = true;
    }

    private DiscordGameEventType(String areaName, DiscordAreaType areaType, int ... regionIds) {
        this.state = DiscordGameEventType.exploring(areaType, areaName);
        this.priority = -2;
    ...
    private DiscordGameEventType(String state, int priority, boolean shouldClear, boolean shouldTimeout, boolean shouldRestart, boolean shouldBeCleared) {
        this.state = state;
        this.priority = priority;
    ...
    private DiscordGameEventType(String state, int priority) {
        this(state, priority, true, false, false, true);
    }

    private static String training(Skill skill) {
        return DiscordGameEventType.training(skill.getName());
    }

    private static String training(String what) {
        return "Training: " + what;
    }

    private static String imageKeyOf(Skill skill) {
        return DiscordGameEventType.imageKeyOf(skill.getName().toLowerCase());
    }

    private static String imageKeyOf(String what) {
        return "icon_" + what;
    }
    ...
    public static DiscordGameEventType fromRegion(int regionId) {
        return FROM_REGION.get(regionId);
    }

    private DiscordGameEventType(@Nullable String imageKey, String state, String details, int priority, boolean shouldClear, boolean shouldTimeout, @Nullable boolean shouldRestart, @Nullable boolean shouldBeCleared, DiscordAreaType discordAreaType, int[] regionIds) {
    ...
    static {
        ImmutableMap.Builder regionMapBuilder = new ImmutableMap.Builder();
        for (DiscordGameEventType discordGameEventType : DiscordGameEventType.values()) {
            if (discordGameEventType.getRegionIds() == null) continue;
            for (int region : discordGameEventType.getRegionIds()) {
                regionMapBuilder.put((Object)region, (Object)discordGameEventType);
            }
        }
